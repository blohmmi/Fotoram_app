...
// Stav aplikace
// ---------------------------------------------------------------------------
let vsechnyPolozky = [];     // posledni seznam ziskany z Androidu (vse, co WhatsApp ma)
...
let aktualniJeVOkne = false; // pouzil se pri sestaveni aktualniho cyklu rezim "jen dnesni"?
...
async function nactiSeznamZeSeveru() {
  try {
    ...
    const pribylyNoveSoubory = novePolozky.some((p) => !stareNazvy.has(p.filename));

    vsechnyPolozky = novePolozky;

    // Pri prijeti nove fotky/videa fronta zahodime, aby se novinka zaradila
    // do prehravani hned a ne az po dobehnuti cele stare fronty.
    if (pribylyNoveSoubory) {
      frontaZobrazeni = [];
    }
  } catch (chyba) {
...
function aktualizujPocitadlo() {
  ...
  const popisek = aktualniJeVOkne ? "nové" : "celkem";
...
function prednactiAZobrazObrazek(polozka) {
  const predbezneNacteni = new Image();
  predbezneNacteni.onload = () => zobrazPolozku(polozka);
